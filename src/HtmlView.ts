...
export class HtmlView extends FileView {
	...
	async onLoadFile(file: TFile): Promise<void> {
		...
		try {
			...
			let dom = null, applyAnchorFix = true, applyDocumentTransform = true;
			switch( this.settings.opMode ) {
				case HtmlPluginOpMode.Native:
					// For trusted local HTML notes, load the vault resource directly.
					// This keeps rendering closest to the system browser and avoids
					// DOMParser/sanitizer/srcdoc rewrites that can alter complex diagrams.
					iframe.sandbox = "allow-same-origin";
					iframe.src = baseHref || this.app.vault.getResourcePath(file);
					applyAnchorFix = false;
					applyDocumentTransform = false;
					break;
			...
			this.mainView.applyDocumentTransform = applyDocumentTransform;
			iframe.onload = async function() {
				...
				if( iframe.mainView.applyDocumentTransform )
...
export async function showError(e: Error | any): Promise<void> {
	...
	// @ts-ignore
	notice.noticeEl.createEl('strong', { text: 'HTML Canvas error' });
